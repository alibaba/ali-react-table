...
基础表格 `BaseTable`是 ali-react-table 中各个表格组件的底层组件，提供了基本的表格渲染能力。BaseTable 只提供了基本的表格渲染能力，一些常见的表格功能（排序、树状模式等）详见 [表格功能](transforms).

:::info
`BaseTabel` 仅包含了基础的表格功能，是一个独立的 React 组件。。。。
:::
...
function 基本用法() {
  ...
  const columns = [
    { code: 'prov', lock: true, name: '省份', width: 150 },
...
除了基本用法之外，`BaseTable` 也支持锁列、表头吸顶、列分组、单元格合并等功能，具体用法可参考 **<LinkTo kind="表格 / 表格示例" story="基本用法">BaseTable 的各个示例</LinkTo>。**

`BaseTable` 的详细 API 详见 **<LinkTo kind="表格 / API" story="page">BaseTable API</LinkTo>。**
