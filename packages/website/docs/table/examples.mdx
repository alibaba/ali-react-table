...
function 数据为空() {
  return (
    <BaseTable
      dataSource={[]}
      columns={assets.mockData.columns2}
...
```jsx live
function 表格数据加载() {
  return <BaseTable isLoading dataSource={assets.mockData.dataSource2} columns={assets.mockData.columns2} />
}
```
...
```jsx live
function 限定表格容器大小() {
  const { dataSource1, operationCol } = assets.mockData
