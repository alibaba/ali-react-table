...
ali-react-table/biz 提供了业务开发中经常需要用到的一些功能，其中的大部分功能采用**统一的拓展机制**进行实现与提供，其他部分功能由导出普通的函数或 React 组件进行提供。

ali-react-table/biz 提供的常用功能列表如下：
...
```javascript
const output = applyTransforms(input, transform_1, transform_2, ...moreTransforms, transform_N)
...
// ... moreTransforms
const output = transform_N(temp2)
```
