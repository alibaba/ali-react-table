...
本页文档将介绍 ali-react-table/biz 提供的排序的相关 API。本页中的示例源码可在 <LinkTo kind="表格功能拓展 / 功能拓展示例" story="">表格功能拓展-功能拓展示例</LinkTo> 查看。
...
如果只想使用 sort 提供的相关交互和视图，但不想让 sort 改变 dataSource，可以将 `features.sortable` 设置为 `() => 0`，这样 sort 就不会对 dataSource 产生影响了。
