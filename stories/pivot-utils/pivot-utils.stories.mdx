...
<Meta title="lab / 透视工具 / 文档" />

# 透视工具

注意：<span style={{color:'darkred'}}>lab 下的组件或工具 API 尚不稳定，请谨慎使用。</span>
...
下钻树用来表示数据按照一定的维度序列被不断分组后形成的树结构。dvt-table 提供了 `buildDrillTree` 方法来从数据中构建下钻树。

### 下钻树构建示例

<Preview>
  <Story id="lab-透视工具-示例--下钻树示例" />
</Preview>
...
很多时候，下钻树与交叉表的 leftTree/topTree 并不是对应的，dvt-table 提供了一些其他方法来在两者之间进行转换。
